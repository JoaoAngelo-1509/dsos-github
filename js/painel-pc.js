...
let _buscaQ = ''; // termo de busca atual — filtro local sobre o array já carregado
...
// ── BUSCA: debounce leve + filtro no cliente ──
let buscaTimer=null;
window.buscaChamados = function(q) {
  _buscaQ = q.trim().toLowerCase();
  clearTimeout(buscaTimer);
  buscaTimer = setTimeout(() => renderChamados(), 120);
};

// ── CARREGA TODOS OS TICKETS (sem filtro de texto no servidor) ──
async function carregarChamados() {
  try {
    const filtroBase = session.tipo === 'professor'
      ? `nome_solicitante=eq.${encodeURIComponent(session.nome)}`
      : `pc_origem=eq.${session.id}`;

    const r = await fetch(
      `${SB_URL}/rest/v1/ticket?${filtroBase}&order=aberto_em.desc&select=*`,
      { headers: H }
    );
...
// ── FILTRO LOCAL — verifica todos os campos relevantes ──
function _matchQ(t, q) {
  if (!q) return true;
  return (
    (t.nome_solicitante || '').toLowerCase().includes(q) ||
    (t.tipo             || '').toLowerCase().includes(q) ||
    (t.laboratorio      || '').toLowerCase().includes(q) ||
    (t.lado             || '').toLowerCase().includes(q) ||
...
function renderChamados() {
  ...
  const visiveis = tickets.filter(t => _matchQ(t, _buscaQ));
  const ativos   = visiveis.filter(t => t.status === 'aberto' || t.status === 'em_andamento').length;

  badge.textContent = ativos || visiveis.length;
  badge.classList.toggle('zero', ativos === 0);

  if (!visiveis.length) {
    const msg = _buscaQ
      ? `Nenhum resultado para "<strong>${_esc(_buscaQ)}</strong>".`
      : (session.tipo === 'professor'
          ? 'Nenhum chamado encontrado para você.<br>Vá em <strong>Abrir Chamado</strong> para criar um.'
          : 'Nenhum chamado encontrado para este PC.<br>Vá em <strong>Abrir Chamado</strong> para criar um.');
    list.innerHTML = `<div class="empty"><div class="eicon"><svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><path d="M16 4h2a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2h2"/><rect x="8" y="2" width="8" height="4" rx="1" ry="1"/></svg></div><p>${msg}</p></div>`;
    return;
  }

  list.innerHTML = visiveis.map(t => {
    ...
    const nlBadge = nl > 0 && podeChat
      ? `<span class="ticket-unread-badge-pc visible"><svg width="9" height="9" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg> ${nl}</span>`
      : '';
...
async function _uploadImg(file) {
  ...
  const res = await fetch(`${SB_URL}/storage/v1/object/chat-prints/${nome}`, {
    method:'POST',
    headers:{'apikey':H.apikey,'Authorization':H.Authorization,'Content-Type':file.type,'x-upsert':'true'},
    body: file
  });
...
window.selecionarImagem = function(event) {
  ...
  const MAX = 5;
  if (imgsPendentes.length + files.length > MAX) { toast(`Máximo de ${MAX} imagens.`,'err'); event.target.value=''; return; }
  files.forEach(file => {
...
window.abrirChamado = async function() {
  ...
  try {
    const r = await fetch(`${SB_URL}/rest/v1/ticket`, {
      ...
      body: JSON.stringify({pc_origem:pcOrigemId, pc_problema:pcProblemaId, tipo, descricao:desc, laboratorio:session.laboratorio, lado:session.lado, status:'aberto', prioridade:'medio', aberto_em:new Date().toISOString(), nome_solicitante:session.nome, chamado_emergencia:emergAtivo||session.tipo==='professor'})
    });
...
window.resetForm = function() {
  ...
  _buscaQ = '';
  const si = document.getElementById('chamados-search'); if (si) si.value = '';
  trocarAba('chamados');
};
